{{-- AI Translation Generator for Articles - Simple Clean Design --}}
<div class="card mt-4">
    ...
    <div class="card-body">
        <div class="alert alert-info">
            ...
            @if(isset($article) && $article->id)
                @if($article->translations && $article->translations->count() > 0)
                    This article has existing translations. Click "View Existing" to see them or "Generate" to create new ones.
                @else
        ...
        <div class="row">
            ...
            <div class="col-md-4 text-end">
                <button type="button" class="btn btn-primary" onclick="generateNewArticleTranslations()">
                    <i class="fas fa-magic" style="margin-right: 8px;"></i>Generate
                </button>
                <button type="button" class="btn btn-info" onclick="viewExistingTranslations()" id="viewExistingTranslationsBtn" style="display:none;">
...
function collectArticleFormContent() {
    const content = {};
    ...
    if (shortDescInput) content.short_description = shortDescInput.value;
    
    const metaTitleInput = document.querySelector('[name="meta_title"]');
    if (metaTitleInput) content.meta_title = metaTitleInput.value;
    
    const metaDescInput = document.querySelector('[name="meta_description"]');
    if (metaDescInput) content.meta_description = metaDescInput.value;
...
function generateNewArticleTranslations() {
    const languages = [];
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            window.generatedTranslations = data.translations;
...
function viewGeneratedTranslations() {
    ...
    // The implementation depends on whether you want to reuse the existing modal
    // or create a new one specifically for articles
    showTranslationModal(window.generatedTranslations);
}

function viewExistingTranslations() {
    if (!window.existingArticleTranslations || Object.keys(window.existingArticleTranslations).length === 0) {
        swal('Info', 'No existing translations found for this article.', 'info');
        return;
    }
    ...
    Object.keys(window.existingArticleTranslations).forEach(locale => {
        const translation = window.existingArticleTranslations[locale];
    ...
    showTranslationModal(formattedTranslations);
}

// Function to be called when translations are applied from the modal
function applyArticleTranslations(translations) {
    window.generatedTranslations = translations;
    document.getElementById('pendingTranslations').value = JSON.stringify(translations);
