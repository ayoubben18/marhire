...
<head>
  <meta charset="UTF-8" />
  <title>MarHire Boat Rental Invoice</title>
  <style>
    body {
...
<body>
  <div class="invoice-box">
    <div class="top-section">
      <div class="top-left">
        <h1>MarHire Boat Rental Invoice</h1>
      </div>
      <div>
    ...
    <p>
      <strong>Boat:</strong> {{ $invoiceData['service_name'] }} – {{ $invoiceData['departure_location'] ?? 'Marina' }}<br />
      @if(isset($invoiceData['rental_duration']))
      <strong>Rental Duration:</strong> {{ $invoiceData['rental_duration'] }}<br />
      @endif
      <strong>Departure:</strong> {{ $invoiceData['departure_location'] ?? 'Marina' }} – {{ $invoiceData['departure_date'] ?? $invoiceData['check_in'] }} at {{ $invoiceData['departure_time'] ?? '10:00' }}<br />
    </p>
    ...
    <table>
      ...
      <tbody>
        <tr><td>Base Rental{{ isset($invoiceData['rental_duration']) && isset($invoiceData['hourly_rate']) ? ' (' . $invoiceData['rental_duration'] . ' × €' . number_format($invoiceData['hourly_rate'], 0) . ')' : '' }}</td><td style="text-align: right;">€{{ number_format($invoiceData['booking_price'], 2) }}</td></tr>
        @if(!empty($invoiceData['addons']))
          ...
          @endforeach
        @endif
        <tr><td>Captain & Fuel Included</td><td style="text-align: right;">Included</td></tr>
      </tbody>
      <tfoot>
    ...
    <div class="terms">
      - Boat rentals must be booked at least 2 days in advance<br />
      - Duration options: hourly, half-day, or full-day depending on the listing<br />
      - Captain and fuel are included unless stated otherwise<br />
      - Please arrive 15 minutes before departure<br />
      - Add-ons and services are subject to availability<br />
    ...
    <div class="footer" style="border-top: 1px solid #ddd; padding-top: 20px; margin-top: 40px;">
      ...
      <p style="margin-top: 10px; font-size: 13px; color: #888;">
        Thank you for booking with <strong style="color: #225f54;">MarHire</strong>. We look forward to welcoming you aboard.
      </p>
    </div>
  </div>
</body>
