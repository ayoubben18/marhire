...
<head>
  <meta charset="UTF-8" />
  <title>Factura Alquiler de Barco MarHire</title>
  <style>
    body {
...
<body>
  <div class="invoice-box">
    <div class="top-section">
      <div class="top-left">
        <h1>Factura Alquiler de Barco MarHire</h1>
      </div>
      <div>
    ...
    <p>
      <strong>Barco:</strong> {{ $invoiceData['service_name'] }} – {{ $invoiceData['departure_location'] ?? 'Puerto deportivo' }}<br />
      @if(isset($invoiceData['rental_duration']))
      <strong>Duración del alquiler:</strong> {{ $invoiceData['rental_duration'] }}{{ is_numeric($invoiceData['rental_duration']) ? ' horas' : '' }}<br />
      @endif
      <strong>Salida:</strong> {{ $invoiceData['departure_location'] ?? 'Puerto deportivo' }} – {{ $invoiceData['departure_date'] ?? $invoiceData['check_in'] }} a las {{ $invoiceData['departure_time'] ?? '10:00' }}<br />
    </p>
    ...
    <table>
      ...
      <tbody>
        <tr><td>Alquiler base{{ isset($invoiceData['rental_duration']) && isset($invoiceData['hourly_rate']) ? ' (' . $invoiceData['rental_duration'] . ' × €' . number_format($invoiceData['hourly_rate'], 0) . ')' : '' }}</td><td style="text-align: right;">€{{ number_format($invoiceData['booking_price'], 2) }}</td></tr>
        @if(!empty($invoiceData['addons']))
          ...
          @endforeach
        @endif
        <tr><td>Capitán y combustible incluidos</td><td style="text-align: right;">Incluido</td></tr>
      </tbody>
      <tfoot>
    ...
    <div class="section-title">Términos de Alquiler y Políticas</div>
    <div class="terms">
      - Los alquileres de barcos deben reservarse con al menos 2 días de antelación<br />
      - Opciones de duración: por horas, medio día, o día completo según el anuncio<br />
      - Capitán y combustible incluidos salvo indicación contraria<br />
      - Por favor llegue 15 minutos antes de la salida<br />
      - Complementos y servicios sujetos a disponibilidad<br />
    ...
    <div class="footer" style="border-top: 1px solid #ddd; padding-top: 20px; margin-top: 40px;">
      ...
      <p style="margin-top: 10px; font-size: 13px; color: #888;">
        Gracias por reservar con <strong style="color: #225f54;">MarHire</strong>. Esperamos darle la bienvenida a bordo.
      </p>
    </div>
  </div>
</body>
