...
<head>
  <meta charset="UTF-8" />
  <title>Facture Réservation d'Activité MarHire</title>
  <style>
    body {
...
<body>
  <div class="invoice-box">
    <div class="top-section">
      <div class="top-left">
        <h1>Facture Réservation d'Activité MarHire</h1>
      </div>
      <div>
    ...
    <p>
      <strong>Activité:</strong> {{ $invoiceData['service_name'] }}<br />
      @if(isset($invoiceData['activity_duration']))
      <strong>Durée:</strong> {{ $invoiceData['activity_duration'] }}<br />
      @endif
      <strong>Date:</strong> {{ $invoiceData['activity_date'] ?? $invoiceData['check_in'] }}@if(isset($invoiceData['time_preference']) && $invoiceData['time_preference']) | <strong>Préférence d'Heure:</strong> {{ ucfirst($invoiceData['time_preference']) }}@endif<br />
      @if(isset($invoiceData['activity_type']))
      <strong>Type:</strong> {{ $invoiceData['activity_type'] }}<br />
      @endif
      @if(isset($invoiceData['number_of_people']) && $invoiceData['number_of_people'] > 0)
      <strong>Nombre de personnes:</strong> {{ $invoiceData['number_of_people'] }}<br />
      @endif
    </p>
    ...
    <table>
      ...
      <tbody>
        <tr><td>Frais d'activité{{ isset($invoiceData['number_of_people']) && $invoiceData['number_of_people'] > 0 && isset($invoiceData['price_per_person']) ? ' (' . $invoiceData['number_of_people'] . ' × €' . number_format($invoiceData['price_per_person'], 0) . ')' : '' }}</td><td style="text-align: right;">€{{ number_format($invoiceData['booking_price'], 2) }}</td></tr>
        @if(!empty($invoiceData['addons']))
          ...
          @endforeach
        @endif
        <tr><td>Prise en charge et guide inclus</td><td style="text-align: right;">Inclus</td></tr>
      </tbody>
      <tfoot>
    ...
    <div class="section-title">Conditions d'Activité et Politiques</div>
    <div class="terms">
      - Les activités doivent être réservées au moins 2 jours à l'avance<br />
      - La prise en charge est incluse si spécifiée dans l'annonce<br />
      - Options et améliorations facultatives selon disponibilité<br />
      - Veuillez être prêt 15 minutes avant l'heure de prise en charge ou de rendez-vous<br />
